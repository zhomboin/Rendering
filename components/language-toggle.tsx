...
function getCompactLocaleLabel(locale: string) {
  return normalizeLocale(locale) === "zh" ? "中" : "EN";
}
...
export function LanguageToggle({ locale }: { locale: string }) {
  ...
  const compactLabel = getCompactLocaleLabel(targetLocale);
  ...
  return (
    <Link aria-label={`${messages.language.label}: ${targetMessages.locale.label}`} className="language-toggle" href={href}>
      ...
      <span aria-hidden="true" className="language-toggle-compact">
        {compactLabel}
      </span>
