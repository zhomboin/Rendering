...
assert.equal(getMessages("zh").site.mobileNavAriaLabel.length > 0, true, "Chinese site messages should expose a mobile nav aria label");
assert.equal(getMessages("en").site.mobileNavAriaLabel.length > 0, true, "English site messages should expose a mobile nav aria label");

for (const locale of ["zh", "en"]) {
  const messages = getMessages(locale);

  assert.equal(typeof messages.home.metadataTitle, "string", `${locale} home messages should expose a metadata title`);
  assert.equal(typeof messages.home.metadataDescription, "string", `${locale} home messages should expose a metadata description`);
  assert.equal(typeof messages.home.hero.kicker, "string", `${locale} home messages should expose hero kicker copy`);
  assert.equal(Array.isArray(messages.home.hero.badges), true, `${locale} home messages should expose hero badges`);
  assert.equal(typeof messages.about.metadataTitle, "string", `${locale} about messages should expose a metadata title`);
  assert.equal(typeof messages.about.metadataDescription, "string", `${locale} about messages should expose a metadata description`);
  assert.equal(Array.isArray(messages.about.profileParagraphs), true, `${locale} about messages should expose profile paragraphs`);
  assert.equal(Array.isArray(messages.about.principles), true, `${locale} about messages should expose principles`);
  assert.equal(typeof messages.article.storyCapsule, "string", `${locale} article messages should expose story capsule copy`);
  assert.equal(typeof messages.article.tocTitle, "string", `${locale} article messages should expose table-of-contents copy`);
  assert.equal(typeof messages.blogArchive.metadataTitle, "string", `${locale} blog archive messages should expose a metadata title`);
  assert.equal(typeof messages.blogArchive.metadataDescription, "string", `${locale} blog archive messages should expose a metadata description`);
  assert.equal(typeof messages.blogArchive.allEssays, "string", `${locale} blog archive messages should expose the all-essays label`);
  assert.equal(typeof messages.blogArchive.filteredSummaryKicker, "string", `${locale} blog archive messages should expose filtered summary copy`);
}
