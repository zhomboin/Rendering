...
assert.equal(localeShellSource.includes("璺冲埌姝ｆ枃"), false, "locale shell should not keep the Chinese skip-link copy");
...
assert.equal(blogIndexPageSource.includes("buildBlogArchiveCopy"), true, "blog archive page should build visible copy from the page-copy helper");
assert.equal(blogIndexPageSource.includes("archiveCopy.guide.title"), true, "blog archive guide copy should come from the page-copy helper");
assert.equal(blogIndexPageSource.includes("archiveCopy.featured.title"), true, "blog archive featured heading should come from the page-copy helper");
