...
const blogDetailPagePath = path.join(process.cwd(), "components", "pages", "blog-detail-page.tsx");
...
const tocPanelPath = path.join(process.cwd(), "components", "toc-panel.tsx");
const readingProgressPath = path.join(process.cwd(), "components", "reading-progress.tsx");
const globalsCssPath = path.join(process.cwd(), "app", "globals.css");
const appLayoutPath = path.join(process.cwd(), "app", "layout.tsx");
const appManifestPath = path.join(process.cwd(), "app", "manifest.ts");
const appIconPath = path.join(process.cwd(), "app", "icon.png");
const publicIconPath = path.join(process.cwd(), "public", "logo", "icon.png");
...
const blogDetailPageSource = fs.readFileSync(blogDetailPagePath, "utf8");
...
const tocPanelSource = fs.readFileSync(tocPanelPath, "utf8");
const readingProgressSource = fs.readFileSync(readingProgressPath, "utf8");
const globalsCssSource = fs.readFileSync(globalsCssPath, "utf8");
const appLayoutSource = fs.readFileSync(appLayoutPath, "utf8");
const appManifestSource = fs.readFileSync(appManifestPath, "utf8");
...
assert.equal(appLayoutSource.includes('icon: "/logo/icon.png"'), true, "root metadata should point icon URLs at the public logo icon");
assert.equal(appLayoutSource.includes('shortcut: "/logo/icon.png"'), true, "root metadata should point shortcut icon URLs at the public logo icon");
assert.equal(appLayoutSource.includes('apple: "/logo/icon.png"'), true, "root metadata should point apple icon URLs at the public logo icon");
assert.equal(appManifestSource.includes('src: "/logo/icon.png"'), true, "web manifest should point at the public logo icon");
assert.equal(appManifestSource.includes('type: "image/png"'), true, "web manifest icon should be declared as a PNG asset");
assert.equal(fs.existsSync(publicIconPath), true, "public logo icon should exist");
assert.equal(fs.existsSync(appIconPath), false, "legacy app/icon.png should be removed once metadata points at the public logo icon");
...
assert.equal(blogDetailPageSource.includes("<ReadingProgress"), false, "blog detail page should stop mounting a separate floating reading-progress pill");
assert.equal(tocPanelSource.includes("ReadingProgress"), true, "toc panel should mount the reading progress bar at the top of the story map card");
assert.equal(readingProgressSource.includes(`role="progressbar"`), true, "reading progress should expose an inline progressbar role for the minimal track");
assert.equal(globalsCssSource.includes(".reading-progress-inline"), true, "styles should define the inline minimal reading-progress bar");
