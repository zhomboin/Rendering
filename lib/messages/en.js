export const enMessages = {
  "locale": {
    "code": "en",
    "label": "English",
    "htmlLang": "en",
    "ogLocale": "en_US"
  },
  "site": {
    "name": "Rendering",
    "brandMark": "Playful Personal Blog",
    "authorName": "Rendering Author",
    "description": "A playful personal blog about frontend systems, reading experience, motion, and design engineering.",
    "mobileNavAriaLabel": "Open navigation menu"
  },
  "navigation": {
    "home": "Home",
    "blog": "Blog",
    "tags": "Tags",
    "about": "About"
  },
  "footer": {
    "copy": "Playful mode is live. Bright on the outside, calm for reading inside.",
    "about": "About",
    "tags": "Tags"
  },
  "language": {
    "label": "Language",
    "zh": "中文",
    "en": "English"
  },
  "theme": {
    "kicker": "Theme",
    "light": "Light",
    "dark": "Dark",
    "toggleToLight": "Switch to light theme",
    "toggleToDark": "Switch to dark theme"
  },
  "search": {
    "trigger": "Search",
    "sectionKicker": "Search Playground",
    "title": "Try searching for something",
    "copy": "",
    "placeholderButton": "Try searching “motion”, “reading”, or “frontend”",
    "openBlogIndex": "Open Blog Index",
    "globalKicker": "Global Search",
    "modalTitle": "Search the playful archive",
    "modalClose": "Close",
    "inputLabel": "Search the blog archive",
    "inputPlaceholder": "Search by topic, mood, or phrase",
    "emptyPrompt": "Type a keyword and see which note rises to the top first.",
    "loading": "Searching the Pagefind index...",
    "noResults": "No matching article turned up this time. Try another phrase.",
    "shortcuts": {
      "focus": "/ focus",
      "move": "↑ ↓ move",
      "open": "Enter open"
    },
    "pills": {
      "pagefind": "Pagefind",
      "archive": "MDX Archive"
    }
  },
  "common": {
    "topic": "Topic",
    "spotlight": "Spotlight",
    "representativeRead": "Representative read",
    "representativePost": "Representative post",
    "openArchive": "Open Archive",
    "openTagArchive": "Open Tag Archive",
    "openTagMap": "Open Tag Map",
    "readLeadArticle": "Read Lead Article",
    "readArchive": "Read Archive",
    "readArticle": "Read Article",
    "readNote": "Read Note",
    "clearFilter": "Clear Filter",
    "backToTagMap": "Back to Tag Map",
    "browseFullArchive": "Browse Full Archive",
    "exploreMoreTags": "Explore More Tags",
    "openFilteredBlogView": "Open Filtered Blog View",
    "moreComing": "More writing is coming soon.",
    "singleArticleCluster": "Single Article Cluster"
  },
  "home": {
    "leadBadge": "Playful mode is live",
    "latest": {
      "title": "Latest drift",
      "copy": "",
      "empty": "Fresh articles are still being staged."
    },
    "topics": {
      "title": "Tag map",
      "copy": "Tap any tag to jump straight into the filtered blog view."
    },
    "about": {
      "title": "About this site",
      "copy": "A playful personal blog about frontend systems, reading experience, motion, and design engineering."
    }
  },
  "about": {
    "leadBadge": "About Rendering",
    "title": "Design reading experiences that feel like tuned systems.",
    "intro": "",
    "body": [
      "This site tracks my long-running interest in frontend systems, reading rhythm, motion restraint, and design engineering.",
      "I like interfaces that keep the spectacle at the edges and leave the center calm enough to actually read."
    ],
    "highlights": {
      "title": "Current focus",
      "items": [
        "Static content systems built with Next.js and MDX",
        "Long-form reading rhythm and information pacing",
        "Interfaces that stay playful without becoming noisy"
      ]
    }
  },
  "blogArchive": {
    "leadBadge": "Blog Archive",
    "heroTitle": "Find the next thing to read through tags, topics, and rhythm.",
    "heroCopy": "",
    "searchTitle": "Search the archive",
    "searchCopy": "",
    "searchPlaceholder": "Search a topic, tool, or phrase",
    "guideTitle": "",
    "guideCopy": "",
    "filterKicker": "Current filter",
    "filterTitle": "",
    "filterCopy": "",
    "featuredTitle": "",
    "featuredCopy": "Start with what is newest or closest to your current interest.",
    "stackTitle": "",
    "stackCopy": "Browse the entire archive by publish date.",
    "emptyForTag": "No public article is published under this tag yet.",
    "clearTag": "Clear tag filter"
  },
  "blogPost": {
    "backToBlog": "Back to blog",
    "storyMap": "Story map",
    "storyMapHint": "Jump through the article by section.",
    "previous": "Previous",
    "next": "Next",
    "related": "Related reads",
    "share": "Share"
  },
  "errors": {
    "notFound": {
      "title": "This page wandered off",
      "copy": "There is nothing public at this address right now. It may not be published yet, or it moved.",
      "backHome": "Back home",
      "backBlog": "Visit the blog"
    },
    "serviceUnavailable": {
      "title": "The site is briefly under maintenance",
      "copy": "A deploy or restart is in progress. Give it a moment and it should be back.",
      "backHome": "Back home",
      "retry": "Try again soon"
    }
  },
  "footerFeed": {
    "label": "RSS Feed",
    "copied": "Copied"
  }
};
